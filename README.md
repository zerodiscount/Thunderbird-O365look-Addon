...
It combines a CSS Theme file (`userChrome.css`) to style the UI, and a custom Add-on (`Outlook-Addon.xpi`) to inject functional buttons into the message list.
...
### Step 1: Install the Outlook Theme (CSS)
...
5. Create a folder named exactly `chrome` (if it does not exist).
6. Copy the `userChrome.css` file from this repository into your new `chrome` folder.
